...
app.post('/api/admin/license', requireAdmin, (req, res) => {
  ...
  const license = { key, memberId: member.id, status: 'active', maxDevices: Math.max(1, Number(body.maxDevices || 1)), devices: [], durationAmount: Number(body.durationAmount || 30), durationUnit: body.durationUnit || 'days', expiresAt, createdAt: nowISO() };
...
app.post('/api/verify', (req, res) => {
  ...
  saveDB(db);
  res.json({ ok:true, message:'License OK', memberName: member.name, gmail: registeredEmail, email: registeredEmail, licenseKey: license.key, expiresAt: license.expiresAt, expiresAtDisplay: formatExpiryForUser(license.expiresAt), maxDevices: license.maxDevices, usedDevices: license.devices.length });
});
app.listen(PORT, () => console.log('Tikfinity Football License Server running at http://localhost:' + PORT));
